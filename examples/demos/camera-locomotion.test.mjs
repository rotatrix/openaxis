...
async function demo(t) {
  ...
  const script = readFileSync(new URL('se2-3d.html', import.meta.url), 'utf8')
...
test('camera locomotion answers z-up facts and lets the profile own constraints', async t => {
  const d = await demo(t);
  const facts = d.query(['world.orientation', 'camera.pose', 'navigation.translation_scale', 'navigation.preferences']).result.values;
  assert.deepEqual(Array.from(facts['world.orientation'].up), [0, 0, 1]);
  assert.equal(facts['navigation.translation_scale'], 10);
  assert.deepEqual({ ...facts['navigation.preferences'] }, { lock_roll: true, lock_translation_plane: true });
  assert.deepEqual(Array.from(facts['camera.pose'].t), [0, 0, 1.5]);
  const h = d.client.handlers;
  const pose = (seq, gesture_id = 1) => ({ type: 'camera.pose', gesture_id, seq, t: [2, 3, 4], r: [.2, .3, .4], fov: 1 });
  h.onMotionStart(1); h.onCameraPose(pose(0));
  assert.equal(d.run('camera.position.x'), 0);
  d.query(['camera.pose'], undefined, 1); h.onCameraPose(pose(1));
  assert.deepEqual(Array.from(d.run('camera.position.toArray()')), [2, 3, 4]);
  Array.from(d.run('cameraPose().r')).forEach((v, i) => assert.ok(Math.abs(v - [.2, .3, .4][i]) < 1e-10));
  assert.ok(Math.abs(d.run('cameraPose().fov') - 1) < 1e-10);
  h.onCameraPose({ ...pose(0), t: [9, 9, 9] });
  h.onCameraPose({ ...pose(2, 99), t: [9, 9, 9] });
  h.onMotionEnd(1); h.onCameraPose({ ...pose(3), t: [9, 9, 9] });
  assert.equal(d.run('camera.position.x'), 2);
});

test('pause, reconnect and page-cache return preserve scene; shutdown disposes shared geometry once', async t => {
  const d = await demo(t);
  ...
  assert.deepEqual(d.client.tags, ['navigation.hint.free_camera']);
  ...
  assert.equal(d.run('fm.lifecycle.state'), 'ready');
  d.run('var releases = 0; geometries[0].addEventListener("dispose", () => releases++);');
  await d.run('shutdown()');
  assert.equal(d.run('releases'), 1);
