---
title: Navigation coordination
description: SDK correction barriers, observation baselines, readback recovery, pose queues and failure boundaries.
---

This reference describes how NavigationSession coordinates camera and object
output. For the purpose of reconciliation and the ordinary integration workflow,
read [Reconciling navigation](/concepts/reconciliation/). Use the
[adapter contract](/reference/navigation-hosts/#writes-and-observations) for
language-specific callbacks and write results.

The SDK owns these coordination rules. Integrations provide native observations,
write results and change notifications; a custom coordinator must maintain the
same ordering and lifetime guarantees.
