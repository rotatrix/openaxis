...
namespace openaxis_ix
{
    ...
    std::pair<bool, std::string> SocketServer::listen()
    {
        ...
        if (_addressFamily == AF_INET)
        {
            ...
            if (openaxis_ix::inet_pton(_addressFamily, _host.c_str(), &server.sin_addr.s_addr) <= 0)
            {
                std::stringstream ss;
        ...
        else // AF_INET6
        {
            ...
            if (openaxis_ix::inet_pton(_addressFamily, _host.c_str(), &server.sin6_addr) <= 0)
            {
                std::stringstream ss;
                ss << "SocketServer::listen() error calling inet_pton "
                   << "at address " << _host << ":" << _port << " : "
    ...
    void SocketServer::run()
    {
        ...
        for (;;)
        {
            ...
            if (_addressFamily == AF_INET)
            {
                ...
                if (openaxis_ix::inet_ntop(AF_INET, &client.sin_addr, remoteIp4, INET_ADDRSTRLEN) == nullptr)
                {
                    int err = Socket::getErrno();
                    std::stringstream ss;
                ...
                remotePort = openaxis_ix::network_to_host_short(client.sin_port);
                remoteIp = remoteIp4;
            }
            else // AF_INET6
            {
                char remoteIp6[INET6_ADDRSTRLEN];
                if (openaxis_ix::inet_ntop(AF_INET6, &client.sin_addr, remoteIp6, INET6_ADDRSTRLEN) ==
                    nullptr)
                {
                    int err = Socket::getErrno();
                    std::stringstream ss;
