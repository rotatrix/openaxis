...
//
// On Windows we use a Windows Event to wake up openaxis_ix::poll() (WSAWaitForMultipleEvents).
// And on any other platform that doesn't support pipe file descriptors we
// emulate the interrupt event by using a short timeout with openaxis_ix::poll() and
