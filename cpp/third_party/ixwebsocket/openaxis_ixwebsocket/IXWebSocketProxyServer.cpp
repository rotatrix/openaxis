...
{
    class ProxyConnectionState : public openaxis_ix::ConnectionState
    {
    public:
        ...
        openaxis_ix::WebSocket& webSocket()
        {
    ...
    private:
        openaxis_ix::WebSocket _serverWebSocket;
        bool _connected;
    };

    int websocket_proxy_server_main(int port,
                                    const std::string& hostname,
                                    const openaxis_ix::SocketTLSOptions& tlsOptions,
                                    const std::string& remoteUrl,
                                    const RemoteUrlsMapping& remoteUrlsMapping,
                                    bool /*verbose*/)
    {
        openaxis_ix::WebSocketServer server(port, hostname);
        server.setTLSOptions(tlsOptions);

        auto factory = []() -> std::shared_ptr<openaxis_ix::ConnectionState> {
            return std::make_shared<ProxyConnectionState>();
        };
        server.setConnectionStateFactory(factory);

        server.setOnConnectionCallback(
            [remoteUrl, remoteUrlsMapping](std::weak_ptr<openaxis_ix::WebSocket> webSocket,
                ...
                state->webSocket().setOnMessageCallback(
                    [webSocket, state, remoteIp](const WebSocketMessagePtr& msg) {
                        if (msg->type == openaxis_ix::WebSocketMessageType::Close)
                        {
                            state->setTerminated();
                        }
                        else if (msg->type == openaxis_ix::WebSocketMessageType::Message)
                        {
                ...
                if (ws)
                {
                    ws->setOnMessageCallback([state, remoteUrl, remoteUrlsMapping](
                        ...
                        if (msg->type == openaxis_ix::WebSocketMessageType::Open)
                        {
                        ...
                        else if (msg->type == openaxis_ix::WebSocketMessageType::Close)
                        {
